...
| 거북목(forwardHead) | 얼굴/어깨 비율 2.5% 증가 + pitch 0.01 증가, 단 카메라 접근이 기준의 ±30% 이내일 때만 |
| 고개 좌우 돌림(yawProxy) | 좌우 비대칭 비율 0.3 (미조정 후보값) |
| 지속 시간 | 1.5초 |
...
- posture features
    ...
    - 예: `shoulderTilt`, `headXOffset`, `shoulderXOffset`, `shoulderYOffset`, `bodyScale`, `motionEnergy`.
- camera state
    - 현재 카메라 환경 판정 결과이다.
...
```
{
  ...
  "features": {
    "shoulderTilt":2.1,
    "headXOffset":0.06,
    "shoulderXOffset":0.52,
    "shoulderYOffset":0.43,
    "bodyScale":1.04,
    "motionEnergy":0.12
  }
}
...
- posture feature 계산
    - shoulder tilt
    - headXOffset
    - shoulderXOffset (어깨 중심 좌우 이동)
    - shoulderYOffset (어깨 중심 높낮이, 기존 headYOffset을 대체)
    - bodyScale

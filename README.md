...
**PostureFairy**는 웹캠으로 사용자의 자세를 실시간 분석하고, 개인별 calibration 기준에서 지속적으로 벗어난 자세가 감지되면 화면 위 요정 알림으로 알려주는 자세 코치입니다.

- **PostureCore**: MediaPipe landmark를 기반으로 feature를 계산하고, 사용자 profile·MAD·자세 Rule·시간 상태를 처리하는 판정 코어
- **웹앱**: Chrome에서 calibration, 실시간 자세 판정, JSONL 세션 녹화와 replay 평가를 실행
- **Electron 앱**: 백그라운드에서도 자세를 감지하고 화면 위 요정 overlay로 알림을 제공하는 데스크톱 버전
- **평가 도구**: 같은 JSONL 로그를 V0(고정 MAD)와 V2(안정 구간 MAD 업데이트)에 replay하여 탐지율, 정상 구간 오탐률, 자세 혼동을 비교

원본 영상과 얼굴 이미지는 저장하지 않습니다. 사용자 profile은 브라우저의 IndexedDB에 저장하고, 평가용 feature·상태·ground truth는 JSONL로 내보냅니다.

...
역할 분담 자체는 [개발 계획](../planning/plan.md) 22절 참고.
...
실제 로직 없이 함수 시그니처와 타입만 만들어둔 뼈대 코드입니다 (`throw new Error("not implemented")`로 표시). 목적은 실제 데이터가 완성되기 전에 다른 사람이 mock 데이터로 병렬 개발을 시작할 수 있게 하는 것 — `docs/planning/plan.md`의 "병렬 진행을 위한 핵심 규칙" 참고.
